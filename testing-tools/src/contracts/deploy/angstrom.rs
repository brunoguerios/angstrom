...
// alloy::providers::WalletProvider<N>,     N: alloy::contract::private::Network
// >(
//     provider: &P,
...
// controller);     let (mock_tob_address, salt) =
//         mine_address(flags, UniswapFlags::mask(), mock_builder.calldata());
//     let final_mock_initcode = [salt.abi_encode(),
// mock_builder.calldata().to_vec()].concat();     RawCallBuilder::new_raw(&
// provider, final_mock_initcode.into())         .to(factory)
//         .gas(50e6 as u64)
//         .send()
//         .await
//         .unwrap()

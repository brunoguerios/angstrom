...
// use alloy::{self, eips::BlockId, network::Network, primitives::Address,
// providers::Provider}; use alloy_primitives::U256;
// use angstrom::components::StromHandles;
// use angstrom_eth::manager::EthEvent;
// use angstrom_network::{PoolManagerBuilder, StromNetworkHandle,
// pool_manager::PoolHandle}; use angstrom_types::{
...
//         angstrom::{
//             AngPoolConfigEntry, AngstromPoolConfigStore,
// AngstromPoolPartialKey,             UniswapAngstromRegistry
//         }
//     },
...
// use validation::{common::TokenPriceGenerator, init_validation,
...
//     let config_store_address = Address::from(<[u8;
// 20]>::try_from(&value_bytes[4..24]).unwrap());     tracing::info!(?
...
//         return Err(eyre!("Invalid encoded entries: must start with a safety
// byte of 0"));     }
...
// ?POOL_CONFIG_STORE_ENTRY_SIZE);         return Err(eyre!("Invalid encoded
...
//                 AngstromPoolPartialKey::new(<[u8;
// 27]>::try_from(&chunk[0..27]).unwrap());             let tick_spacing =
// u16::from_be_bytes([chunk[27], chunk[28]]);             let fee_in_e6 =
// u32::from_be_bytes([0, chunk[29], chunk[30], chunk[31]]);             (
//                 pool_partial_key,
//                 AngPoolConfigEntry {
//                     pool_partial_key,
//                     tick_spacing,
...
// pub async fn initialize_strom_components_at_block<Provider:
// WithWalletProvider>(     handles: StromHandles,
...
//     let signer =
// AngstromSigner::for_address(telemetry_constants.node_address());
...
//         SubmissionHandler::new(provider.rpc_provider().into(), &[], &[], &[],
// angstrom_contract);
...
//     let (_eth_event_tx, eth_event_rx) =
// tokio::sync::mpsc::unbounded_channel::<EthEvent>();
//     let eth_event_rx_stream = UnboundedReceiverStream::new(eth_event_rx);
//     let (_eth_event_tx_pmb, eth_event_rx_pmb) =
// tokio::sync::mpsc::unbounded_channel::<EthEvent>();
//     let eth_event_rx_stream_pmb =
// UnboundedReceiverStream::new(eth_event_rx_pmb);     // let eth_data_sub =
...
// already     // created
//     let network_stream =
//         Box::pin(eth_event_rx_stream) as Pin<Box<dyn Stream<Item = EthEvent>
// + Send + Sync>>;
...
//     executor.spawn_critical("uniswap pool manager",
// Box::pin(uniswap_pool_manager));     // EXTERNAL DATA - reads the price
// history from the chain to establish the price     // background. Can be
// snapshotted or re-read from the chain     let price_generator =
// TokenPriceGenerator::new(         provider.rpc_provider().into(),
//         block_id,
//         uniswap_pools.clone(),
//         gas_token,
//         Some(1)
...
//     // Needs to regularly talk to the chain, this one is complicated.
// However,     // given a single block and no actual updates to the canonical
// state we should     // be able to freeze this in time at a specific block
...
//         // Because this is incapsulated under the orderpool syncer. this is
// the only case         // we can use the raw stream.
//         update_stream,
//         uniswap_pools.clone(),
//         price_generator,
//         pool_config.clone(),
...
//     let network_handle =
// StromNetworkHandle::new(Arc::new(AtomicUsize::new(1)), sn_tx);
...
//     executor.spawn_critical_with_graceful_shutdown_signal("consensus", move
// |grace| {         manager.run_till_shutdown(grace)
//     });

...
// pub type Provider = BlockchainProvider<NodeTypesWithDBAdapter<EthereumNode,
// Arc<DatabaseEnv>>>;

// pub fn load_reth_db(db_path: &Path) -> Provider {
//     let db = Arc::new(
//         reth_db::open_db_read_only(db_path,
// DatabaseArguments::new(ClientVersion::default()))             .unwrap()
//     );
...
//         StaticFileProvider::read_only(static_files, true).expect("static file
// provider failed");

//     let provider_factory =
//         ProviderFactory::new(db.clone(), Arc::clone(&chain),
// static_file_provider);     let executor =
// reth_node_ethereum::EthExecutorProvider::ethereum(provider_factory.
// chain_spec());     let tree_externals = TreeExternals::new(
...
//     let blockchain_tree =
//         ShareableBlockchainTree::new(BlockchainTree::new(tree_externals,
// tree_config).unwrap());

//     BlockchainProvider::new(provider_factory.clone(),
// Arc::new(blockchain_tree)).unwrap() }

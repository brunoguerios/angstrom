...
//     pool::EnhancedUniswapPool, pool_data_loader::DataLoader,
...
//     let mut pool = EnhancedUniswapPool::new(DataLoader::new(address),
// ticks_per_side);     tracing::info!(block_number = block_number, "loading old
// pool");     pool.initialize(Some(block_number), provider.clone())
//         .await?;
//     pool.set_sim_swap_sync(true);
//
//     let state_change_buffer = 1;
//
//     let mock_block_stream = Arc::new(MockBlockStream::new(provider.clone(),
// from_block, to_block));     let pools = vec![pool];
//
//     let sync = MockBlockSync;
//
//     let uniswap_pool_manager =
//         UniswapPoolManager::new(pools, block_number, state_change_buffer,
// mock_block_stream, sync);
//
//     let (mut rx, _join_handles) =
// uniswap_pool_manager.subscribe_state_changes().await?;
//
...
// {                     let mut fresh_pool =
// EnhancedUniswapPool::new(DataLoader::new(address), ticks_per_side);
//                     fresh_pool.initialize(Some(changes_block_number),
// provider.clone()).await?;
//
//                    let pool_guard =
// uniswap_pool_manager.pool(&address).unwrap();                     // Compare
// the new pool with the old pool                     compare_pools(&pool_guard,
// &fresh_pool, changes_block_number);                 } else {
//                     // Channel closed
//                     break;
//                 }
//             }
...
// }
//
// fn compare_pools(old: &EnhancedUniswapPool, new: &EnhancedUniswapPool,
// block_number: BlockNumber) {     let mut differences_found = false;
...
//     // will show that there is a difference in bitmap, but that won't be
// relevant     // because left/right tick end would be -/+1 for the fresh pool
...
//             diff
//         );
//         if let (Some(old_min), Some(new_min)) = (old.ticks.keys().min(),
// new.ticks.keys().min()) {             tracing::warn!(
//                 block_number = block_number,
//                 "Min tick:   Old: {} | New: {}",
//                 old_min,
//                 new_min
//             );
//         }
//         if let (Some(old_max), Some(new_max)) = (old.ticks.keys().max(),
// new.ticks.keys().max()) {             tracing::warn!(
//                 block_number = block_number,
//                 "Max tick:   Old: {} | New: {}",
//                 old_max,
//                 new_max
//             );
//         }
//         let original_ticks: std::collections::HashSet<_> =
// old.ticks.keys().collect();         let fresh_ticks:
// std::collections::HashSet<_> = new.ticks.keys().collect();         let
// left_diff: Vec<_> = fresh_ticks.difference(&original_ticks).collect();
//         let right_diff: Vec<_> =
// original_ticks.difference(&fresh_ticks).collect();         if
// !left_diff.is_empty() {             tracing::warn!(
//                 block_number = block_number,
...
//     // fresh pool that are not in the old simply because left/right end would
// be     // -/+1 for the fresh pool
...
//         tracing::error!(block_number=block_number, address=?old.address(),
// "differences found between pools");     } else {
//         tracing::info!(block_number=block_number, address=?old.address(),
// "pools are the same");     }
// }
//
...
//     fn sign_off_reorg(&self, _: &'static str, _: RangeInclusive<u64>, _:
// Option<Waker>) {}
//

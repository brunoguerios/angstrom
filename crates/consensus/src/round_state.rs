...
// }
//
// type TransitionFuture = BoxFuture<'static, Result<ConsensusState,
// RoundStateMachineError>>;
//
...
//     }
//
//     pub fn my_pre_proposal(&self, pre_proposals: &HashSet<PreProposal>) ->
// Option<StromMessage> {         pre_proposals
//             .iter()
...
//                 // it does not make sense to accumulate pre_proposals here,
// since the leader                 // already submitted on chain
//                 if matches!(self.current_state,
// ConsensusState::Finalization(_))                     ||
...
//
//                 // we do not want to allow another node to push us to
// transition                 // we wait for our grace period of 3 seconds to
// finish                 if !matches!(self.current_state,
// ConsensusState::PreProposalAggregation(_)) {                     return None
//                 }
//
//                 if !i_am_leader {
//                     let block_height = self.current_state.block_height();
//                     let merged_pre_proposal =
// Self::generate_our_merged_pre_proposal(                         block_height,
//                         Vec::new(),
//                         Vec::new(),
...
//
//                     if
// self.have_quorum(self.all_searcher_orders(pre_proposals))
...
//                 let Proposal {
//                     source: proposal_sender, block_height:
// proposal_block_height, ..                 } = proposal;
//
//                 let pre_proposals = self.current_state.pre_proposals();
//                 if proposal.is_valid() && !i_am_leader {
//
// self.force_transition(ConsensusState::Finalization(Finalization {
// block_height:  proposal_block_height,                         proposal:
// Some(proposal),                         pre_proposals: pre_proposals.clone()
//                     }));
//                 }
//
//                 if i_am_leader {
//                     panic!(
//                         "Message sender: {}, Proposal block height: {},
// Proposal source: {}, \                          Current state {}, something
// is terribly wrong",                         msg_sender,
...
//         let OrderSet { limit, searcher } =
// self._order_storage.get_all_orders();         let mut pre_proposals =
// pre_proposals.clone();
//
...
//             .into_values()
//             .filter_map(|group| group.into_iter().max_by_key(|order|
// order.tob_reward))             .collect();
//
...
//     }
//
//     fn have_quorum<O: Hash + Eq + Clone>(&self, orders:
// Vec<OrderWithStorageData<O>>) -> bool {         orders.len() ==
// self.filter_quorum_orders(orders).len()     }
//
...
//         self.transition_future =
// Some(Box::pin(self.build_transition_future(new_state)));
//
//         // wake up the poller
...
//
//         async move {
//             if let ConsensusState::Finalization(finalization) = &mut
// new_state {                 // someone already proposed and we are not a
// leader                 if finalization.proposal.is_some() {
//                     // TODO: use this opportunity to trigger the proposal
// validation                     return Ok(new_state)
//                 }
//
...
//
//                         Some((*key, (token_a, token_b, snapshot,
// entry.store_index as u16)))                     })
//                     .collect::<HashMap<_, _>>();
//
//                 let (proposal, timer) = async_time_fn(|| async {
//                     match matching
//                         .solve_pools(pre_proposals.clone(),
// pool_snapshots.clone())                         .await
//                     {
//                         Ok((solutions, gas_info)) => {
//                             let proposal =
//                                 signer.sign_proposal(pre_proposal_height,
// pre_proposals, solutions);                             Ok((proposal,
// gas_info))                         }
//                         Err(err) =>
// Err(RoundStateMachineError::ProposalBuildError(err))                     }
//                 })
//                 .await;
...
//
//                 let bundle =
//                     AngstromBundle::from_proposal(&proposal, gas_info,
// &pool_snapshots).unwrap();
//
...
// Poll<Option<Self::Item>> {         let this = self.get_mut();
//

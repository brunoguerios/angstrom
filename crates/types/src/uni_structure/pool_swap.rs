...
impl<'a> PoolSwap<'a> {
    ...
    pub fn swap_with_cmp_hook(
        mut self,
        break_early: impl Fn(U256, U256, U256) -> bool
        ...
        while amount_remaining != I256::ZERO && sqrt_price_x96 != sqrt_price_limit_x96 {
            ...
            // This logic here allows more granularity when swapping allowing to get swap
            // stats as its happening to allow for additional functionality.
            let (tmp_d0, tmp_d1) = if self.direction {
                let this_total_in = total_in + amount_in + fee_amount;
                let this_total_out = total_out + amount_out;
                (this_total_in, this_total_out)
            } else {
                let this_total_in = total_in + amount_in + fee_amount;
                let this_total_out = total_out + amount_out;
                (this_total_out, this_total_in)
            };

            if break_early(new_sqrt_price_x_96, tmp_d0, tmp_d1) {
                break;
            }

...
impl GroupedVanillaOrder {
    ...
    /// Provides a price that, post fee scaling, will be equal to this price.  Used for matching.
    pub fn pre_fee_price(&self, fee: u128) -> Ray {
        self.price().unscale_to_fee(fee)
    }

    /// Provides the fee-adjusted price to be used for accounting.  Note that for bids this can be inverse

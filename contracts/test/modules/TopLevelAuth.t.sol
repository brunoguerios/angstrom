...
contract TopLevelAuthTest is BaseTest {
    ...
    function test_batchUpdatePools() public {
        ...
        (StoreKey key, uint16 tickSpacing, uint24 bundleFee, uint24 unlockedFee, uint24 protocolFee) = getEntry(0);
    ...
    function getEntry(uint256 index)
        internal
        view
        returns (StoreKey key, uint16 tickSpacing, uint24 bundleFee, uint24 unlockedFee, uint24 protocolFee)
    {
        PoolConfigStore store = angstrom.configStore();
